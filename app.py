...
# English Comment: Define the structure of the incoming request body (input data for the model)
class Item(BaseModel):
    features: list[float]
    # Example: features: list[float] = [0.1, 0.2, 0.3, 0.4]

# English Comment: Define the structure of the prediction response
...
# English Comment: This function runs ONCE when the Uvicorn server starts up
...
async def load_model_on_startup():
    ...
    # English Comment: Simple check to ensure the model file exists
    if not os.path.exists(MODEL_PATH):
        print(f"ERROR: Model file not found at {MODEL_PATH}. Exiting.")
        return
        
    try:
        # English Comment: FIX: Allow PyTorch to safely load custom class (required since PyTorch 2.6 security update)
        # We must allow the custom class 'SimpleClassifier' to be unpickled.
        torch.serialization.add_safe_globals([SimpleClassifier])
        
        # English Comment: Load the entire model object onto the CPU.
        ...
        model.eval() # English Comment: Set the model to evaluation mode
        print(f"SUCCESS: PyTorch model loaded from {MODEL_PATH}")
        
    except Exception as e:
        print(f"ERROR loading PyTorch model: {e}")
        model = None # English Comment: Ensure model is None if loading failed
...
async def predict(item: Item):
    ...
    try:
        # 1. English Comment: Convert input list (from Item.features) to a PyTorch tensor
        input_tensor = torch.tensor([item.features], dtype=torch.float32)
        
        # 2. English Comment: Perform inference
        with torch.no_grad():
            output = model(input_tensor)
        
        # 3. English Comment: Convert output tensor back to a standard Python list
        prediction_list = output.tolist()[0]
        
        return {"prediction_result": prediction_list}
        
    except Exception as e:
        # English Comment: Log the error and return a safe message
